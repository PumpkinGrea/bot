...
- **聊天**：`@咱 + 文字` DeepSeek V4 Flash 对话，带上下文记忆；`@咱 清空对话` 重置记忆
- **今日运势**：`@咱 今日运势`，同人同天结果固定
- **随机二次元图**：`@咱 来张图` / `二次元`
- **Pixiv 搜图**：`@咱 搜索P站 标签`，按标签随机取一张 Pixiv 全年龄作品（需配置图片服务）
- **AI 生图**：`@咱 画图 描述`（如「画图 戴帽子的猫」）；可附最多 4 张参考图
- **查卡组**：`@咱 查卡组 [轮换/无限] [卡组类型]`，从 [SVWB Meta](https://svwbmeta.com/) 获取公开高分天梯构筑与卡表
- **卡组梯度**：`@咱 卡组梯度 [轮换/无限]`，查看 [SVWB Meta](https://svwbmeta.com/) 当前梯度表
- **查卡条件筛选**：`@咱 查卡 皇221`，按职业、费用、攻击、生命查从者；职业简称为妖/皇/法/龙/梦/教/鱼，对应全称也可用。`皇221` 即皇家护卫 2 费 2/1，10 费等两位数字请写为 `皇家护卫 10 2 1`
- **查游戏王卡**：`@咱 查游戏王卡 卡名`，查询中文卡名、日英文名、种类、数值、效果与官方密码，例如 `@咱 查游戏王卡 青眼白龙`
- **猜卡**：`@咱 猜卡`，随机截取卡图猜名称；10 分钟内用 `@咱 猜 卡名` 作答，可发送 `猜卡提示` 或 `换一张猜卡`
...
- `config_secret.py`：DeepSeek 中转站、生图中转站的 API key（**含密钥，不入库**）
...
镜像/幻影坦克/Pixiv 搜图要让公网能拉到本机生成或转存的图。本地开发用内网穿透（如 cpolar）：
...
QQ 开放平台 ── WebSocket ──> bot.py (botpy 事件循环)
            ...
            （运势/工具/    （DeepSeek 文本对话） acg/gpt_draw   （本机起 HTTP
